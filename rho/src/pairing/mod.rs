...
//!
//! This module builds the bilinear-pairing substrate on top of the frozen
//! `shared::field` prime-field arithmetic.  It introduces no changes to
//! `rho::curve` or `shared::field` — it composes them (option-B decision).
//!
//! # Pairing design statement
//!
...
//!
//! **MOV readiness.**  [`weil::weil_pairing`] and [`tate::reduced_tate`] both
//! return [`fpext::FpExt`] values — elements of `F_{p^k}` represented as
//! coefficient vectors over `F_p`.  The MOV bridge in [`mov`] reads these as
//! `F_{p^k}` elements (via `to_uint_vec`-style canonicalisation) and maps them
//! toward `solve_dl`.  The [`fpext::FpExt::frobenius`] map is over-specified
//! here (the pairings themselves barely use it) because the MOV reduction needs
//! it centrally.
//!
//! # Module tree
//!
//! - [`fpext`] — `F_{p^k}` extension-field arithmetic.
//! - [`ecext`] — `E(F_{p^k})` point arithmetic.
...
//! - [`miller`] — Miller's algorithm + line/vertical functions.
//! - [`weil`] — Weil pairing: two Miller calls, no final exponentiation.
//! - [`tate`] — Tate/reduced-Tate pairing: one Miller call + final
//!   exponentiation `^{(p^k−1)/ℓ}`.
